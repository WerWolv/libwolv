...
namespace wolv::util {
    ...
    std::string_view utf8Substr(std::string_view input, size_t start, size_t length) {
        auto inputLength = utf8StringLength(input);
        ...
        for (size_t utf8Index = 0; utf8Index < start; ++utf8Index) {
            utf8Start += utf8CharLength(input[utf8Start]);
        }

        size_t utf8Length = 0;
        for (float utf8Index = 0; std::ceil(utf8Index) < static_cast<float>(length); utf8Index += 1.0f) {
            size_t charLength = utf8CharLength(input[utf8Start + utf8Length]);
            std::string_view charView = input.substr(utf8Start + utf8Length, charLength);
            if (isCJKGlyph(charView))
                utf8Index += 0.75f;

            utf8Length += charLength;
        }
        return input.substr(utf8Start, utf8Length);
    }
